...
type GroupQuery struct {
	...
	withDocumentAcls *DocumentACLQuery
...
// QueryDocumentAcls chains the current query on the "document_acls" edge.
func (_q *GroupQuery) QueryDocumentAcls() *DocumentACLQuery {
...
func (_q *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withDocumentAcls: _q.withDocumentAcls.Clone(),
...
// WithDocumentAcls tells the query-builder to eager-load the nodes that are connected to
// the "document_acls" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *GroupQuery) WithDocumentAcls(opts ...func(*DocumentACLQuery)) *GroupQuery {
	query := (&DocumentACLClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withDocumentAcls = query
	return _q
}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withDocumentAcls; query != nil {
		if err := _q.loadDocumentAcls(ctx, query, nodes,
			func(n *Group) { n.Edges.DocumentAcls = []*DocumentACL{} },
			func(n *Group, e *DocumentACL) { n.Edges.DocumentAcls = append(n.Edges.DocumentAcls, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *GroupQuery) loadDocumentAcls(ctx context.Context, query *DocumentACLQuery, nodes []*Group, init func(*Group), assign func(*Group, *DocumentACL)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Group)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(documentacl.FieldGroupID)
	}
	query.Where(predicate.DocumentACL(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.DocumentAclsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
