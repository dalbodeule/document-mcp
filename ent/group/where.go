...
// HasDocumentAcls applies the HasEdge predicate on the "document_acls" edge.
func HasDocumentAcls() predicate.Group {
	return predicate.Group(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
...
// HasDocumentAclsWith applies the HasEdge predicate on the "document_acls" edge with a given conditions (other predicates).
func HasDocumentAclsWith(preds ...predicate.DocumentACL) predicate.Group {
	return predicate.Group(func(s *sql.Selector) {
		step := newDocumentAclsStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
