...
type UserQuery struct {
	...
	withDocumentsCreated *DocumentQuery
...
// QueryDocumentsCreated chains the current query on the "documents_created" edge.
func (_q *UserQuery) QueryDocumentsCreated() *DocumentQuery {
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withDocumentsCreated: _q.withDocumentsCreated.Clone(),
...
// WithDocumentsCreated tells the query-builder to eager-load the nodes that are connected to
// the "documents_created" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithDocumentsCreated(opts ...func(*DocumentQuery)) *UserQuery {
	query := (&DocumentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withDocumentsCreated = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withDocumentsCreated; query != nil {
		if err := _q.loadDocumentsCreated(ctx, query, nodes,
			func(n *User) { n.Edges.DocumentsCreated = []*Document{} },
			func(n *User, e *Document) { n.Edges.DocumentsCreated = append(n.Edges.DocumentsCreated, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadDocumentsCreated(ctx context.Context, query *DocumentQuery, nodes []*User, init func(*User), assign func(*User, *Document)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(document.FieldCreatedByUserID)
	}
	query.Where(predicate.Document(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.DocumentsCreatedColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
